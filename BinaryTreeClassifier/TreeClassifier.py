...
class TreeClassifier:
    ...
    def __init__(self, max_depth=None, min_samples_split=2, splitting_criterion=None, feature_expansion=None):

        if splitting_criterion is None:
            raise TypeError("'splitting_criterion' must be not None")

        if feature_expansion is None:
            feature_expansion = lambda x: x
    ...
    def _best_split(self, X, y):
        ...
        for i in X.columns.values:  # Loop on all the features
            thresholds = np.unique( self.feature_expansion(X[i]) )
            if(len(thresholds)<1):
                continue
